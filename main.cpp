...
int main(int argc, char *argv[]){
  ...
  enum COMMAND {AddOrder, DeleteOrders, SearchByDate, 
                SearchByDateRank, CheckCorrectness, Halt};
  COMMAND cmd(Halt);
  ...
  while(testCase >> s){
    if(cmd != Halt){
      ...
      switch(cmd){
        case AddOrder:
          MGMTSystem.addOrder(arg, atoi(s.c_str())); break;
        case DeleteOrders:
          MGMTSystem.deleteOrders(arg, atoi(s.c_str())); break;
        case SearchByDate:
          your_list = MGMTSystem.searchByDate(arg, atoi(s.c_str())); break;
        case SearchByDateRank:
          your_list = MGMTSystem.searchByDateRank(arg, atoi(s.c_str())); break;
        case CheckCorrectness:
          if(compare(testANS, golden_list, your_list)){++pass_times;} 
		  ++cf_times; break;
        default:
    ...
    else if(s == "AddOrder") { cmd = AddOrder; } 
    else if(s == "DeleteOrders"){ cmd = DeleteOrders;} 
    else if(s == "SearchByDate"){ cmd = SearchByDate;}
    else if(s == "SearchByDateRank"){ cmd = SearchByDateRank;}
    else if(s == "CheckCorrectness"){ cmd = CheckCorrectness;}
  }
  ...
  cout << "Correctness = " 
       << static_cast<double>(100*pass_times/cf_times) << "%" << endl;
  return 0;
}

bool compare(ifstream &testANS, list<unsigned> &golden_list, list<unsigned> &your_list){
  golden_list.clear();
  string s;
  while(testANS >> s){
    if(s ==">"){
      while (testANS >> s){
        if(s == ";") break;
        else golden_list.push_back(atoi(s.c_str()));
      }
      break;
    }
  }
